...
library LibStays {
    ...
    bytes32 private constant DATETIME_TYPEHASH =
        keccak256('DateTime(uint16 yr,uint8 mon,uint8 day,uint8 hr,uint8 min,uint8 sec)');
    bytes32 private constant STAYS_TYPEHASH =
        keccak256(
            'Stay(DateTime checkIn,DateTime checkOut,uint32 numPaxAdult,uint32 numPaxChild,uint32 numSpacesReq)DateTime(uint16 yr,uint8 mon,uint8 day,uint8 hr,uint8 min,uint8 sec)'
        );
    ...
    function hashStaysDeal(
        ...
        return keccak256(abi.encode(STAYS_TYPEHASH, bid, params, dst));
    }
    ...
    function hash(DateTime memory a) internal pure returns (bytes32) {
        return keccak256(abi.encode(DATETIME_TYPEHASH, a.yr, a.mon, a.day, a.hr, a.min, a.sec));
    }
