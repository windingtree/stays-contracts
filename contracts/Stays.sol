...
contract Stays is Context, EIP712 {
  ...
  function rely(address usr) external auth { require(live == 1, "Stays/not-live"); wards[usr] = 1; }
  function deny(address usr) external auth { require(live == 1, "Stays/not-live"); wards[usr] = 0; }
  modifier auth {
  ...
  function deal(
    ...
    if (options.items.length > 0) {
      (stubState.items, stubState.cost) = LibVidere.addOptions(
        stubState.items,
        options.items,
        stubState.cost
      );
    ...
    if (options.terms.length > 0) {
      (stubState.terms, stubState.cost) = LibVidere.addOptions(
        stubState.terms,
        options.terms,
        stubState.cost
      );
